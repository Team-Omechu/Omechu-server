  ...
  try {
    ...
    const formattedZzimList = slicedZzims.map(zzim => {
      ...
      if (restaurant.review.length > 0) {
        ...
        averageRating = Math.round((totalRating / restaurant.review.length) * 10) / 10;
      }

      // 대표 메뉴 추출
      const representativeMenus = restaurant.repre_menu.map(item => item.menu);
...
export const findUserReviews = async (userId, limit, cursor) => {
  try {
    ...
    const nextCursor = hasNextPage ? slicedReviews[slicedReviews.length - 1].id : null;
